...
const DistributorTable = () => {
    ...
    const fetchHistory = async (limit = 10, page = 0, search = '', path) => {
        try {
            setShowLoader(true);
            const response = await GET(`/distributor`);
            setShowLoader(false);
    ...
    return (
        <React.Fragment>
            <Typography variant={'h2'} mb={3}>Distributors</Typography>
            ...
            {showLoader ? <DistributorTableItem retailer={null} key={0} showLoader={true} /> : <></>}
            {history.length > 0 ? (showLoader ? <></> :
                history.map((retailer, index) => <DistributorTableItem retailer={retailer} key={index} fetchHistory={fetchHistory} />)
            ) : (showLoader ? <></> :
                <StyledCard>
