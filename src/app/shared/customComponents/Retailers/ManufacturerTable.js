...
import DistributorTableHeader from './DistributorTableHeader';
import DistributorTableItem, { StyledCard } from './DistributorTableItem';
import ManufacturerTableItem from './ManufacturerTableItem';
...
const ManufacturerTable = () => {
    ...
    const fetchHistory = async (limit = 10, page = 0, search = '', path) => {
        try {
            setShowLoader(true);
            const response = await GET(`/manufacturer`);
            setShowLoader(false);
    ...
    return (
        <React.Fragment>
            <Typography variant={'h2'} mb={3}>Manufacturers</Typography>
            ...
            {showLoader ? <ManufacturerTableItem retailer={null} key={0} showLoader={true} /> : <></>}
            {history.length > 0 ? (showLoader ? <></> :
                history.map((retailer, index) => <ManufacturerTableItem retailer={retailer} key={index} fetchHistory={fetchHistory} />)
            ) : (showLoader ? <></> :
                <StyledCard>
