...
const DistributorTableItem = ({ retailer, showLoader, fetchHistory }) => {

    const navigate = useNavigate()

    const handleSideMenus = (action) => {
       if(action.action === "viewDistributor"){
        navigate(`/users/distributor/${retailer._id}`)
       }
    };
            ...
            <Stack direction={"row"} alignItems={"center"} sx={{ p: theme => theme.spacing(2, 1) }}>
                ...
                <Item sx={{ ml: { xs: 'auto', sm: 0 } }}>
                    <JumboDdMenu
                        icon={<MoreHorizIcon />}
                        menuItems={[
                            { icon: <Visibility />, title: "View", action: "viewDistributor" },
                        ]}
