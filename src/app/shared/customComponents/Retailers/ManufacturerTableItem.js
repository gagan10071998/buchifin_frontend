...
const ManufacturerTableItem = ({ retailer, showLoader, fetchHistory }) => {

    const navigate = useNavigate()

    const handleSideMenus = (action) => {
       if(action.action === "viewManufacturer"){
        navigate(`/users/manufacturer/${retailer._id}`)
       }
    };
            ...
            <Stack direction={"row"} alignItems={"center"} sx={{ p: theme => theme.spacing(2, 1) }}>
                ...
                <Item sx={{ ml: { xs: 'auto', sm: 0 } }}>
                    <JumboDdMenu
                        icon={<MoreHorizIcon />}
                        menuItems={[
                            { icon: <Visibility />, title: "View", action: "viewManufacturer" },
                        ]}
...
export default ManufacturerTableItem;
